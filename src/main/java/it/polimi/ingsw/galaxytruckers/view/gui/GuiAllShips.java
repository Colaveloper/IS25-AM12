...
import it.polimi.ingsw.galaxytruckers.view.cli.CliShipBoard;
            ...
            componentProperty.addListener((obs, oldVal, newVal) -> {
                Platform.runLater(() -> {
                    componentContainer.getChildren().setAll(
                            newVal != null
                                    ? new GuiComponent(model, server, newVal).getNode()
                                    : new Label("?"));
                });
            });

            // Initial content
            GuiComponent initial = new GuiComponent(model, server, componentProperty.get());
            componentContainer.getChildren().add(initial != null ? initial.getNode() : new Label("?"));

            GuiShipBoard shipBoard = new GuiShipBoard(model, server, color);
